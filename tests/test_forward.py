...
def test_ISWTGradg(iswtoperator):
    ...
    if iswtoperator.setting == "synthesis":
        ...
        f_wav_lm = np.zeros(
            [(iswtoperator.L) ** 2, iswtoperator.nscales], dtype=np.complex
        )
        vlen = pys2let.mw_size(iswtoperator.L)
        for j in range(iswtoperator.nscales):
            f_wav_lm[:, j] = pys2let.map2alm_mw(
                f_wav[j * vlen : (j + 1) * vlen + 1], iswtoperator.L, 0
            )
...
def test_SWC2PixGradg(swc2pixoperator):
    ...
    if swc2pixoperator.setting == "analysis":
        ...
        f_wav_lm = np.zeros(((L) ** 2, swc2pixoperator.nscales), dtype=np.complex)
